...
_main() {
    ...
    ARCH="amd64"
    FILENAME="yq_linux_${ARCH}"
    URL="$REPO_URL/releases/download/v${VERSION:?No version specified}/${FILENAME}"
    TARGET_DIR="${HOME}/.local/opt/yq-v${VERSION}/bin"
    TARGET_LINK_DIR="${HOME}/.local/bin"
    TARGET_BIN="yq"
    _info "Downloading and installing" "$TARGET_BIN" ...
    mkdir -p "${TARGET_DIR}" || exit 1
    curl -#Lo "${TARGET_DIR}/${TARGET_BIN}" "$URL" || exit 1
